...
! $ In January 2024 I changed this routine by the one below.
 function iran_bin_num_rec(n,pp)
 implicit double precision (a-h,o-z)
...
2    y=dtan(pi*dran_u())
 ...
 return
end function iran_bin_num_rec

!!$ Generation of binomial distribution
!!$ Copied from BNLDEV of Numerical Recipes and "translated" to modern fortran of double precision
!!$ Modified to use dran routines
!!$ Modified in January 2024 according to the discussion in the binomial.tex file
!!$ Also the line en=n has been removed and en has been replaced by n whenever it appears, and we use
!!$ directly define em as integer and set em=floor(sq*y+am) checking 0<=em<=n, instead of  em=sq*y+a, checking 0<em<n+1 and then em=int(em)

function iran_bin(n,pp)
  implicit double precision (a-h,o-z)
  integer em
  parameter (pi=3.1415926535897932385d0)
  SAVE nold,pold,pc,plog,pclog,en,oldg,a1
  DATA nold /-1/, pold /-1.0d0/
  ...
  if ((n.eq.0).or.(pp.eq.0.0d0)) then !Trivial cases
    iran_bin=0
  else if (p.lt.0.15d0.and.am.lt.15.0d0) then !!$ USE INVERSION
     if (p.ne.pold.or.n.ne.nold) a1=(1.0d0-p)**n  !!$ Do not recompute a1 unless p or n have changed
     a2=a1
     u=dran_u()
     i=0
     do  while(a2.lt.u)
        a1=a1*p/(1.0d0-p)*(n-i)/(i+1.0d0)
        a2=a2+a1
        i=i+1
        if (i.gt.n) exit !! This should never happen
     enddo
     iran_bin=i
  else if (p.gt.0.15d0.and.n.lt.100) then !!$ USE BERNOUILLI
     iran_bin=0
     do  j=1,n
        if(dran_u().lt.p) iran_bin=iran_bin+1
     enddo
  else !!$ USE REJECTION ACCORDING TO NUMERICAL RECIPES
     if (n.ne.nold) then !!$ Do not call gammln routine if the argument has not changed
        oldg=gammln(n+1.0d0)
        nold=n
     endif
     if (p.ne.pold) then !!$ Do not recompute logs if the argument has not changed
        pc=1.0d0-p
        plog=dlog(p)
        pclog=dlog(pc)
        pold=p
     ...
     sq=dsqrt(2.0d0*am*pc)
2    y=dtan(pi*dran_u())
     em=floor(sq*y+am)  !!$ Proposal
     if (em.lt.0.or.em.gt.n) go to 2
     t=1.20d0*sq*(1.0d0+y**2)*dexp(oldg-gammln(em+1.0d0)-gammln(n-em+1.0d0)+em*plog+(n-em)*pclog)
     if (dran_u().gt.t) go to 2  !!$ Rejection
     iran_bin=em
  endif
  if (p.ne.pp) iran_bin=n-iran_bin
  return
...
!!$  x=xx  !!$ Before it said function gammln(xx), double precision xx and x=xx. I do not think this is necessary
  y=x
  tmp=x+5.5d0
